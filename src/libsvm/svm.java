...
public class svm {
	...
	//edited by ShiChaohao
	public static double gamma = Integer.MAX_VALUE;
	public static double rho = Integer.MAX_VALUE;
	public static double c = Integer.MAX_VALUE;
	public static svm_node[] v;
	public static svm_node[][] M;
	...
	public static double svm_predict_values(svm_model model, svm_node[] x, double[] dec_values)
	{
		...
		{
			if (rho == Integer.MAX_VALUE){
				File file = new File("ijcnn1-g0.04.model.deal");
				try {
					Scanner input = new Scanner(file);
					while(input.hasNextLine()){
						String line = input.nextLine();
						if (line.contains("gamma")){
							gamma = Double.parseDouble(line.split(":")[1]);
						}
						if (line.contains("rho")){
							rho = Double.parseDouble(line.split(":")[1]);
						}
						if (line.contains("c")){
							c = Double.parseDouble(line.split(":")[1]);
						}
						if (line.contains("v")){
							break;
						}
					...
					v = new svm_node[temp.length];
					for (int key = 0; key < temp.length; ++key){
						v[key] = new svm_node();
					}
					for (int j = 0; j < temp.length; ++j){
						v[j].index = Integer.parseInt(temp[j].split(":")[0]);
						v[j].value = Double.parseDouble(temp[j].split(":")[1]);
					}
					input.nextLine();//È¥³ýM
					M = new svm_node[124][124];
					for (int i1 = 0; i1 < M.length; ++i1){
						for (int j1 = 0; j1 < M[i1].length; ++j1){
							M[i1][j1] = new svm_node();
						}
					}
					int row = 0;
					while(input.hasNextLine()){
						...
						for (int col = 0; col < temp1.length; ++col){
							M[row][col].index = Integer.parseInt(temp1[col].split(":")[0]);
							M[row][col].value = Double.parseDouble(temp1[col].split(":")[1]);
						}
						++row;
						}
					input.close();
			...
			sum += vectorMul(v, x);
			sum += matrixMul(M, x);
//			System.out.println(matrixMul(M, x));
			sum *= Math.exp(-gamma * vectorMul(x, x));
			sum -= rho;
			if (sum > 0)
...
//			return model.label[vote_max_idx];
	...
	public static double matrixMul(svm_node[][]x, svm_node[]y){
		svm_node[] z = new svm_node[124];
		svm_node[] m = new svm_node[124];
		...
		for (int i = 0; i < 124; ++i){
			...
			for (int j = 0; j < 124; ++j){
				m[j].index = j + 1;
				m[j].value = x[j][i].value;
			}
//			for (int k = 0; k < y.length; ++k){
//				System.out.print(y[k].index + ":" + y[k].value + " ");
//			}
//			for (int k = 0; k < m.length; ++k){
//				System.out.print(m[k].index + ":" + m[k].value + " ");
//			}
//			System.out.println("");
			sum = vectorMul(y, m);
			z[i].index = i;
...
//		for (int i = 0; i < z.length; ++i)
//			System.out.println(z[i].index + ":" + z[i].value);
		return vectorMul(z, y);
	}
	
	public static double vectorMul(svm_node[]x, svm_node[]y){
		double sum = 0;
		int xlen = x.length;
		int ylen = y.length;
		int i = 0;
